...
from app.services.ml.radimagenet_labels import CT_LABELS, label_for_index
...
MODEL_VERSION = "Lab-Rasool/RadImageNet · 1.35M CT/MR/US radiology images · 165 pathology classes"
...
def _load_model() -> tuple[nn.Module, torch.device]:
    ...
    with _lock:
        ...
        model.eval()
        _model = model
        logger.info("RadImageNet CT model ready.")
        return _model, _device
...
def _ct_class_indices() -> set[int]:
    indices = set()
    for i, name in enumerate(CT_LABELS):
        if name:
            indices.add(i)
    # RadImageNet CT classes occupy first portion of unified label space
    if not indices:
        indices = set(range(0, 55))
    return indices
...
def analyze(png_bytes: bytes, threshold: float | None = None) -> ModelAnalysisResult:
    ...
    ct_indices = _ct_class_indices()
    candidates = []
    raw_scores = {}
    for idx in range(len(probs)):
        name = label_for_index(idx)
        raw_scores[name] = round(float(probs[idx]), 4)
        if idx in ct_indices or any(k in name.lower() for k in ("ct", "chest", "lung", "hepatic", "abdom", "pelvis", "covid", "consolidation", "nodule")):
            if probs[idx] >= threshold * 0.5:
    ...
    for idx, name, conf in candidates[:5]:
    ...
    if findings:
        try:
            ...
            logger.warning("CT Grad-CAM failed: %s", exc)

    return ModelAnalysisResult(
        findings=findings[:4],
        heatmap=heatmap,
        model_name=MODEL_NAME,
        model_version=MODEL_VERSION + " · CT pathway",
