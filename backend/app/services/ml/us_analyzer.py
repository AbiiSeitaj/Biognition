...
MODEL_ID = "Parveshiiii/breast-cancer-detector"
MODEL_NAME = "Breast Ultrasound Classifier (ViT)"
MODEL_VERSION = "Parveshiiii/breast-cancer-detector · BUSI dataset · benign/malignant/normal"
...
def _load_model():
    ...
    with _lock:
        ...
        _model.eval()
        logger.info("Ultrasound model ready.")
        return _model, _processor, _device
...
def _attention_heatmap(model, inputs, image_size: tuple[int, int]) -> np.ndarray | None:
    try:
        ...
        if attn.max() > 0:
            attn = attn / attn.max()
        img = Image.fromarray((attn * 255).astype(np.uint8))
        img = img.resize(image_size, Image.Resampling.BILINEAR)
        return np.array(img, dtype=np.float32) / 255.0
    except Exception as exc:
        logger.warning("US attention map failed: %s", exc)
        return None
...
def analyze(png_bytes: bytes, threshold: float | None = None) -> ModelAnalysisResult:
    ...
    for i in range(len(probs)):
        key = str(i)
        label_name = id2label.get(key, id2label.get(i, f"class_{i}"))
        raw_scores[label_name] = round(float(probs[i]), 4)
    ...
    for idx, conf in ranked[:3]:
        ...
        if conf < threshold and name == "normal":
            continue
        if conf < threshold * 0.6 and findings:
            continue
        findings.append(
            ModelFinding(
                ...
                severity=US_SEVERITY.get(name, infer_severity(name, conf)),
                class_index=idx,
            )
        )
    ...
    if not findings:
        ...
        findings.append(
            ModelFinding(
                ...
                region=infer_region(name),
                severity=US_SEVERITY.get(name, infer_severity(name, best_conf)),
